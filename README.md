...
| `e2e-player.cjs` | **动作演示播放器**（读 canvas 像素验证解码、自动循环无定格、步骤联动） |
...
- **剔掉定格帧。** `motionFrameIndices()` 按「明显长于中位数」识别定格帧并排除，
  播放与步骤映射都基于筛出来的运动帧序列，动作因此是连贯的。
  代价是节奏不再等同于原素材 —— 但原素材本来也没表达真实运动速度
  （真实的俯卧撑不会每组之间停半秒），这个取舍是划算的。
- **步骤与画面对齐。** 点第 k 条步骤直接跳到对应帧，播放时反向高亮当前步骤
   （取**距离最近**的步骤，不是「不超过当前帧的最后一步」—— 后者在循环回绕处
   会让高亮从末步弹回第 1 步）。
- **降级**：解码失败回落到静态缩略图；用户开了 `prefers-reduced-motion` 则不自动播放。

### 为什么刻意没有播放控制条

做过一版带暂停 / 逐帧 / 慢放 / 进度条 / 速度档的播放器，**被否掉了**。

用户的原话是「不太想要暂停 / 逐帧 / 慢放、控制条」。回头看这是对的：
需求是「打开就看到动作在动、且看得懂」，不是「能操控」。一排控件占掉画面下方的空间，
是负担而不是功能。而慢放想解决的问题（看不清动作）已经由**剔除定格帧**解决了 ——
那是让动作连贯的**根本原因**，跟有没有控制条无关。

所以演示区保持安静：自动循环、不打断、不需要操作。唯一保留的交互是「点步骤跳画面」，
因为它解决的是「看不懂」，而不是「不好用」。

**改播放器请跑 `node e2e-player.cjs`**（先起 4199）。控制条去掉后测法也得跟着变：
没有按钮可点，所以改用**定时采样** canvas 像素指纹，检查画面是否一路推进 ——
`最长静止 ≤ 3 次采样（约 480ms）`这条专门守着定格帧剔除逻辑。已实测注入回归
（把 `motionFrameIndices` 改成直接返回全部帧）时它会报「最长静止 6 次 ≈ 0.96s」并失败，
说明断言是真的有牙的。

...
| `e2e-player.cjs` | **动作演示区**（读 canvas 像素验证解码、零交互、无定格、原速节奏） |
...
- **剔掉定格帧。** `motionFrameIndices()` 按「明显长于中位数」识别定格帧并排除，
  只循环运动帧序列。**这是动作连贯的唯一保障。**
- **播放节奏 = 原素材原速**（`SLOWDOWN = 1`）。中间一度放慢到 2 倍（100ms → 200ms），
  想让人看清关节角度；用户试过之后要求「再加快一点」，于是回到原速。
  常量留着而不是写死，因为万一以后再要调速度只是改一个数字。
- **降级**：解码失败回落到静态缩略图；用户开了 `prefers-reduced-motion` 则不自动播放。

> 早先的注释里写过「先剔除定格帧、再整体放慢，顺序不能反」。现在没有放慢了，
> 但那个约束依然成立，而且**原速下剔除定格帧变得更关键** ——
> 一旦定格帧漏网，立刻退回「动一下停一秒」的原始状态，没有任何缓冲。
...
3. 现在这样：**动图自己循环，下面单纯列出步骤文字。零交互。**

所以演示区里没有任何可点元素：没有控制条，步骤列表也退化成纯文字（`<li>` 而非按钮）。
唯一还保留的机制是「剔除定格帧」，它在代码里，不占用界面。
...
- **零交互** —— 断言 `.player` 内部的可点元素数为 0。这条看着像废话，
  ...
  而不是等用户抱怨。
- **定格帧剔除 + 节奏** —— 在**页面内**用 `requestAnimationFrame` 观测画面变化，
  `最长一次静止` 守着前者、`帧间隔中位数` 守着后者（原速期望 ≈100ms）。

已实测两条都有牙（注入回归后确实失败）：

| 注入的回归 | 断言反应 |
| --- | --- |
| `SLOWDOWN = 2`（又放慢了） | 帧间隔中位数 **201ms** → 失败（期望 70–160ms） |
| `motionFrameIndices` 直接返回全部帧 | 最长一次静止 **1842ms** → 失败 |

速度断言的区间是 `[70, 160]`，**改 `SLOWDOWN` 必须同步改它** ——
这不是麻烦，是它的作用：逼人确认「调速度」是有意的，而不是手滑。

⚠️ 这里有个**测法陷阱**值得记下：最初我在 Node 侧循环
`取指纹 → waitForTimeout(40)` 来量帧间隔，量出 47ms。
其实那是**测法错了**：每次取指纹都要走一遍 Playwright 跨进程协议，
单次往返远超 40ms，于是真正的采样间隔由协议开销决定，
算出来的「间隔」跟动画速度毫无关系。
把观测放进页面内（rAF 与渲染同频、时间戳同一个时钟）才量到真实值。
**测不准时先怀疑测法，别急着改代码。**

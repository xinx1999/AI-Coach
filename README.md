...
| `e2e-player.cjs` | **动作分解播放器**（读 canvas 像素验证解码、逐帧、慢放、步骤联动） |
...
- **自己解码、自己按帧重绘。** 用 `omggif` 把 GIF 拆成「每帧 RGBA + 原始延时」，
  ...
  也就是说原生 `<img>` 永远做不到慢放，也拿不到暂停/步进的接口。
- **剔掉定格帧。** `motionFrameIndices()` 按「明显长于中位数」识别定格帧并排除，
  播放、进度条、步骤映射都基于筛出来的运动帧序列，动作因此是连贯的。
  代价是节奏不再等同于原素材 —— 但原素材本来也没表达真实运动速度
  （真实的俯卧撑不会每组之间停半秒），这个取舍是划算的。
- **步骤与画面对齐。** 点第 k 条步骤直接跳到对应帧并停住，播放时反向高亮当前步骤。
  这是「看懂动作」的关键 —— 以前文字和画面各走各的，用户只能自己脑补对应关系。
- **降级**：解码失败回落到静态缩略图；用户开了 `prefers-reduced-motion` 则不自动播放。

**改播放器请跑 `node e2e-player.cjs`**（先起 4199）。它读 canvas 像素断言每帧内容真的不同，
其中「步进时画面每次都变」这条专门守着定格帧剔除逻辑 —— 早先直接对帧号 ±1 时，
用户按「下一帧」有大概率停在几乎相同的画面上，会以为按钮坏了。
